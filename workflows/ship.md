...
1. **Setup the environment for testing** by running `make ship-prepare` to ensure that the environment is cleanly loaded and restarted. This is all you need to do to prepare the environment for UAT. Do not do any further investigation or validation of the environment unless explicitly asked.
...
- When resetting the local repo back to main ready for working on a new issue, execute git commands individually so it's clearer what you're doing and applying an AllowList to the commands is easier.
