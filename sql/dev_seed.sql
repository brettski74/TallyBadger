...
INSERT INTO parties (name, role, is_active, subtype, default_revenue_account_id, default_expense_account_id)
SELECT 'Charles Checkbook', 'customer', TRUE, 'tenant', (SELECT id FROM accounts WHERE LOWER(name) = LOWER('Rent Revenue') LIMIT 1), NULL::bigint
WHERE NOT EXISTS (SELECT 1 FROM parties p WHERE LOWER(p.name) = LOWER('Charles Checkbook'));
