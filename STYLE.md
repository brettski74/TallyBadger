...
| **UI Conventions** | Standard Keyboard Shortcuts | Ctrl+S - save shortcut. For forms that have a **Save** button or something else save-like that applies current form state to the database, Ctrl+S keyboard acts the same as if clicking that button. Ctrl+Shift+D - discard shortcut. For forms that have a discard, cancel or similar button that discards current changes and possibly closes the form, pressing Ctrl+Shift+D acts the same as if clicking that button. These keyboard shortcuts should be implemented on any appropriate forms for consistency throughout the application. |
