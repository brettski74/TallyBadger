"""Service for named cheque register list filter presets (#196)."""
...
class ChequeRegisterFilterPresetConflictError(Exception):
    """Raised when a preset name already exists."""


class ChequeRegisterFilterPresetNotFoundError(Exception):
    """Raised when a preset id does not exist."""


def _row_to_out(row: dict[str, Any]) -> ChequeRegisterFilterPresetOut:
    ...
    return ChequeRegisterFilterPresetOut(
        id=int(row["id"]),
        name=str(row["name"]),
        definition=ChequeRegisterFilterPresetDefinition.model_validate(definition),
        created_at=row["created_at"],
        updated_at=row["updated_at"],
    )


class ChequeRegisterFilterPresetService:
    def __init__(
        self,
    ...
    def list_presets(self) -> list[ChequeRegisterFilterPresetOut]:
        with self._connection_factory() as conn:
            with conn.cursor(row_factory=dict_row) as cur:
                cur.execute(
                    """
                    SELECT id, name, definition, created_at, updated_at
                    FROM cheque_register_filter_presets
                    ORDER BY name ASC
                    """
                )
                rows = cur.fetchall()
    ...
    def get_preset(self, preset_id: int) -> ChequeRegisterFilterPresetOut:
        with self._connection_factory() as conn:
            with conn.cursor(row_factory=dict_row) as cur:
                cur.execute(
                    """
            ...
            try:
                with conn.transaction():
                    with conn.cursor(row_factory=dict_row) as cur:
                        cur.execute(
                            """
                            INSERT INTO cheque_register_filter_presets (name, definition)
                            VALUES (%s, %s::jsonb)
            ...
            try:
                with conn.transaction():
                    with conn.cursor(row_factory=dict_row) as cur:
                        cur.execute(
                            """
                            UPDATE cheque_register_filter_presets
                            SET
        ...
        with self._connection_factory() as conn:
            with conn.transaction():
                with conn.cursor() as cur:
                    cur.execute(
                        "DELETE FROM cheque_register_filter_presets WHERE id = %s",
                        (preset_id,),
                    )
                    if cur.rowcount == 0:
                        raise ChequeRegisterFilterPresetNotFoundError(
