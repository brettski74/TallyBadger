...
**Line ↔ allocation linkage (#270, #277):** each applied obligation amount has one or more **journal lines** on that `entry_id` (bridge A/R, A/P, unearned, or prepaid role account; or multiple **cash** lines after multi-cash accrual collapse). `journal_lines.settlement_allocation_id` points at the corresponding `settlement_allocations` row (`ON DELETE SET NULL`; multiple lines may share one allocation id). PostgreSQL enforces **`SUM(ABS(amount))` over linked lines = `settlement_allocations.amount`** at commit via deferrable constraint triggers (migration `034`). `GET /journal-entries/:id` exposes `settlement_allocations` on the entry and `obligation_id` on each linked line via that FK — no heuristic matching.
