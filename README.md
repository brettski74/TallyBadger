...
- **Expanded snapshot checksums:** after editing table JSON or `attachments/*` files in an expanded directory (or adding a new `member_manifest` entry by hand), run **`mkmeta`** (`scripts/mkmeta`, stdlib-only) to refresh **`member_manifest`** SHA-256 digests in **`metadata.json`**. Default updates the file in place (timestamped backup of the previous file); **`--check`** verifies without writing. Then restore with **`tbload`** as usual.
