...
| Import rules (#8) | **CEL** rules: **`evaluate_cel`**, persisted rule sets (`/import-rules/cel/rule-sets`), **`POST /import-rules/cel/evaluate`**, CSV execute with `cel_rule_set_id`; **Import rules** tab in the SPA for editing sets — **see [docs/import-rules-engine.md](docs/import-rules-engine.md)** (matcher/action `evaluate` + `POST /import-rules/evaluate` remain for evaluation only, not CSV) |
