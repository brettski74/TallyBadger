...
| Config | `tallybadger.core.config.Settings` — `TALLYBADGER_DATABASE_URL` (see below); **`TALLYBADGER_PDF_PAGE_SIZE`** (`us-letter` default, or `a4`) for financial report PDF exports; journal **attachment** max upload size lives on **`ledger_settings.max_attachment_upload_bytes`** (default **5 MiB**), adjustable via `PATCH /ledger-settings` using a byte integer or a string with **`k`** (×1024) or **`M`** (×1048576) |
...
- **API:** `POST /backup/export` streams **`application/gzip`** (`.tar.gz`). `POST /backup/import` accepts a **raw body** upload with **`restore_mode`** as a query parameter. Container (ZIP vs tar.gz) is detected from **magic bytes**, not filename. OpenAPI: `/docs`.
...
- **Expanded snapshot checksums:** after editing table JSON or `attachments/*` files in an expanded directory (or adding a new `member_manifest` entry by hand), run **`mkmeta`** (`scripts/mkmeta`, stdlib-only) to refresh **`member_manifest`** SHA-256 digests in **`metadata.json`**. By default, **`mkmeta`** updates the file in place (timestamped backup of the previous file) when any digest is missing or stale; if everything already matches, it exits without writing. **`--change` / `-c`** writes only when **`--input` and `--output` differ** or at least one digest would change (useful for in-place runs that must not touch the file when digests are already current). **`-r`** keeps at most three timestamped backups after a write. **`--check`** verifies without writing. Then restore with **`tbload`** as usual.
...
4. **Import:** `tbload --mode erase-reload -i /tmp/tb-snap.tar.gz` or `curl -fsS -X POST --data-binary @/tmp/tb-snap.tar.gz -H "Content-Type: application/gzip" "http://127.0.0.1:8080/backup/import?restore_mode=erase-reload"` (see format spec for legacy ZIP `Content-Type`). Invalid modes such as `erase-spice-girls-music` are rejected. Mode prefixes (`a`, `o`, `e`, `erase-`) resolve to the single matching canonical value.
